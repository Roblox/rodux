...
Thunks are the simplest way to introduce more complex processing of `action` objects, but if used to excess, can make your code difficult to follow. However, thunks allow any portion of your code with access to the `store` to create custom business logic and dispatch it via an `action` to the `store`. One of the primary goals of Rodux is to put your business logic in a centralized location and thunks work directly against that goal. It is often a good first step for prototyping a new idea, but it's highly recommended that you create custom [`middleware`](middleware.md) for complex features instead of relying on thunks alone.